...
int ImageCap::openCap()
{
    ...
    imageFormat.fmt.pix.height = 320;
...
int ImageCap::takePicture(const std::string& img_dest)
{
    // Queue the buffer
    ...
    cout << "Buffer has: " << (double)bufferinfo.bytesused / 320
            << " KBytes of data" << endl;

    // Write the data out to file
    ofstream outFile;
    outFile.open(img_dest.c_str(), ios::binary| ios::app);
    ...
    while(remainingBufferSize > 0) {
        ...
        outFileMemBlockSize = 320;    // set the output block size to a preferable size. 1024 :)
